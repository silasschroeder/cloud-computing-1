...
# Output the master node IP for easy access
output "master_ip" {
...
output "worker_ips" {
  value = openstack_compute_instance_v2.worker_nodes[*].network.0.fixed_ip_v4
  description = "IP addresses of worker nodes"
}

output "application_url" {
  value = "http://${openstack_compute_instance_v2.master.network.0.fixed_ip_v4}:30000"
  description = "URL to access the deployed application"
}

output "ssh_command" {
  value = "ssh ubuntu@${openstack_compute_instance_v2.master.network.0.fixed_ip_v4}"
